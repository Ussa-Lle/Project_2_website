...
<xsl:template match="racine/FILTRAGE/item">
    <xsl:for-each select="./titre/article/element">
        <xsl:if test="./data[contains(@type,'type')]='ADJ' and preceding-sibling::element[1]/data[contains(@type,'type')][contains(text(),'DET')] and following-sibling::element[1]/data[contains(@type,'type')][contains(text(),'NOM')]">
